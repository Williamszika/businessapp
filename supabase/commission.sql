-- =====================================================================
-- ZKA — Commissions (commercial 5 %, responsable 2 %), réglables par le président
...
update public.profiles set comm = 0.02 where role = 'responsable'  and coalesce(comm, 0) = 0;
...
begin
  if new.comm is null or new.comm = 0 then
    new.comm := case new.role when 'commercial' then 0.05 when 'responsable' then 0.02 else 0 end;
  end if;
  return new;
end; $$;
